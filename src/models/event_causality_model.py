...
class EventCausalityEngine:
    ...
    def _analyze_causality_pair(
        ...
        if (
            time_lag.total_seconds() < 0
            or time_lag.total_seconds() > self.max_time_lag_hours * 3600
        ):
            return None
        ...
        causal_patterns = {
            ...
            ("evidence_discovery", "legal_action"): (
                CausalityType.CONTRIBUTING_FACTOR,
                0.3,
            ),
        }
    ...
    def simulate_impact_propagation(
        self, source_event_id: str, events: List[Dict[str, Any]], max_depth: int = 5
    ) -> EventImpact:
        ...
        # Find source event
        source_event = next(
            (e for e in events if e["event_id"] == source_event_id), None
        )
        if not source_event:
            return None
            ...
            for event_id in current_level:
                ...
                for rel in self.causal_relationships.values():
                    if rel.cause_event_id == event_id:
                        ...
                        effect_event = next(
                            (e for e in events if e["event_id"] == rel.effect_event_id),
                            None,
                        )
                        if effect_event:
                            affected_entities.update(
                                effect_event.get("participants", [])
                            )
        ...
        impact_level = self._calculate_impact_level(
            len(affected_events), len(affected_entities)
        )
    ...
    def _calculate_impact_level(
        self, num_events: int, num_entities: int
    ) -> ImpactLevel:
    ...
    def _detect_linear_sequences(
        ...
        for participant, participant_evts in participant_events.items():
            if len(participant_evts) >= 3:
                sorted_evts = sorted(
                    participant_evts, key=lambda e: e.get("timestamp", datetime.min)
                )
    ...
    def _detect_branching_sequences(
        self, events: List[Dict[str, Any]]
    ) -> List[EventSequence]:
        ...
        for event in events:
            ...
            for other_event in events:
                ...
                if other_time and event_time and other_time > event_time:
                    ...
                    for rel in self.causal_relationships.values():
                        if (
                            rel.cause_event_id == event["event_id"]
                            and rel.effect_event_id == other_event["event_id"]
                        ):
    ...
    def _detect_cyclical_sequences(
        self, events: List[Dict[str, Any]]
    ) -> List[EventSequence]:
    ...
    def detect_anomalies(self, events: List[Dict[str, Any]]) -> List[EventAnomaly]:
        ...
        for event in events:
            ...
            has_causal_rel = any(
                rel.cause_event_id == event["event_id"]
                or rel.effect_event_id == event["event_id"]
                for rel in self.causal_relationships.values()
            )
    ...
    def predict_future_events(
        self, events: List[Dict[str, Any]], prediction_window_days: int = 30
        ...
        for sequence in self.event_sequences.values():
            if sequence.sequence_type == EventSequenceType.CYCLICAL:
                ...
                last_event = next(
                    (e for e in events if e["event_id"] == sequence.events[-1]), None
                )
                if last_event:
                    ...
                    if last_time:
                        ...
                        for i in range(len(sequence.events) - 1):
                            e1 = next(
                                (
                                    e
                                    for e in events
                                    if e["event_id"] == sequence.events[i]
                                ),
                                None,
                            )
                            e2 = next(
                                (
                                    e
                                    for e in events
                                    if e["event_id"] == sequence.events[i + 1]
                                ),
                                None,
                            )
                            if e1 and e2:
                        ...
                        if time_diffs:
                            ...
                            if predicted_time <= datetime.now() + timedelta(
                                days=prediction_window_days
                            ):
                                prediction = EventPrediction(
                                    prediction_id=f"pred_{sequence.sequence_id}",
                                    predicted_event_type=last_event.get(
                                        "event_type", "unknown"
                                    ),
    ...
    def export_analysis(self) -> Dict[str, Any]:
        ...
        return {
            "causal_relationships": [
                rel.dict() for rel in self.causal_relationships.values()
            ],
