...
class Agent(BaseModel):
    ...
    def add_goal(
        ...
        goal = AgentGoal(
            description=description, priority=priority, target_state=target_state or {}
        )
        self.goals.append(goal)
        return goal

    def add_behavior_rule(
        self, condition: str, action: str, priority: float = 0.5
    ) -> BehaviorRule:
        """Add a behavioral rule"""
        rule = BehaviorRule(condition=condition, action=action, priority=priority)
...
class MultiAgentSimulation:
    ...
    def create_agent(
        ...
        agent = Agent(name=name, agent_type=agent_type, behavior_type=behavior_type)
        ...
        source_agent.learn_from_experience(
            {
                ...
                "outcome": (
                    "positive" if impact_on_source.get("benefit", 0) > 0 else "negative"
                ),
    ...
    def _determine_interaction_outcome(
        self, source: Agent, target: Agent, interaction_type: InteractionType
    ) -> str:
        ...
        if interaction_type == InteractionType.COLLABORATION:
            if (
                source.behavior_type == BehaviorType.COOPERATIVE
                and target.behavior_type == BehaviorType.COOPERATIVE
            ):
    ...
    def _calculate_interaction_impacts(
        self,
        source: Agent,
        target: Agent,
        interaction_type: InteractionType,
    ...
    def _apply_interaction_impacts(
        self, source: Agent, target: Agent, interaction: AgentInteraction
    ):
        """Apply interaction impacts to agent states"""

        # Update source agent
        source_resources = source.current_state.resources.copy()
        source_resources["influence"] = (
            source_resources.get("influence", 0)
        ...
        target_resources["influence"] = (
            target_resources.get("influence", 0)
        ...
        relationship_change = (
            0.1 if interaction.outcome in ["mutual_success", "compromise"] else -0.1
        )
        ...
        for _ in range(num_steps):
            ...
            for agent in self.agents.values():
                ...
                context = {
                    ...
                    "other_agents": [
                        a for a in self.agents.values() if a.agent_id != agent.agent_id
                    ],
                }
    ...
    def _detect_coalitions(self) -> List[List[str]]:
        ...
        for agent_id, agent in self.agents.items():
            ...
            ) in agent.current_state.relationships.items():
    ...
    def export_simulation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "emergent_patterns": [
                pattern.dict() for pattern in self.emergent_patterns.values()
            ],
