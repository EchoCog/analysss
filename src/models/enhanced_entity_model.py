...
class EnhancedEntity(BaseModel):
    ...
    def validate_behavioral_properties(cls, v):
        for key, value in v.items():
            ...
            if not 0.0 <= value <= 1.0:
                raise ValueError(
                    f"Behavioral property {key} must be between 0.0 and 1.0"
                )
        return v

    def create_version(
        self, changed_by: Optional[str] = None, change_reason: Optional[str] = None
    ):
    ...
    def update_property(
        self, property_name: str, value: Any, changed_by: Optional[str] = None
    ):
        """Update a property and create a version"""
        if hasattr(self, property_name):
            setattr(self, property_name, value)
            self.create_version(
                changed_by=changed_by, change_reason=f"Updated {property_name}"
            )
        else:
    ...
    def calculate_similarity(self, other: "EnhancedEntity") -> float:
        ...
        if self.tags and other.tags:
            tag_overlap = len(self.tags.intersection(other.tags)) / max(
...
class EntityManager:
    ...
    def find_entities_by_tag(self, tag: str) -> List[EnhancedEntity]:
        ...
        return [
            self.entities[eid] for eid in self.entity_index["by_tag"].get(tag, set())
        ]

    def find_similar_entities(
        self, entity_id: str, threshold: float = 0.5
    ) -> List[Tuple[EnhancedEntity, float]]:
